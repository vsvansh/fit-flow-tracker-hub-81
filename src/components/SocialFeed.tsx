            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {[
                  ...
                  { id: 3, user: "Emma Wilson", avatar: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8M3x8dXNlcnxlbnwwfHwwfHx8MA%3D%3D", activity: "shared a new workout routine", time: "Yesterday" },
                  { id: 4, user: "Michael Brown", avatar: "https://images.unsplash.com/photo-1639149888905-fb39731f2e6c?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTl8fHVzZXJ8ZW58MHx8MHx8fDA%3D", activity: "joined the 30-Day Challenge", time: "2 days ago" },
                ].map((activity, index) => (
                  <div key={index} className="p-3 border rounded-md flex items-start">
                    ...
                    <Button 
                      variant="ghost" 
                      size="sm"
                      className={`transition-colors ${thumbsUp[activity.id] ? 'text-green-600 hover:text-green-700' : ''}`}
                      onClick={() => handleThumbsUp(activity.id)}
                    >
                      <ThumbsUp className={`h-4 w-4 transition-transform ${thumbsUp[activity.id] ? 'scale-125' : ''}`} />
                    </Button>
                  </div>
                ))}
              </div>
